...
# 1- Get_Devices
# 2- Show Devices
# 3- Show_Devices_Config
#
# Syntax:
# GET:
# /network-device/{startIndex}/{recordsToReturn}
# /network-device/count
# /network-device/1/10     == get device 1 to 10
# /network-device/<network device id>/config
#
...
# definitions
#
programmname = "./gui_get_device_config.py"
apic_url = "https://sandboxapic.cisco.com/api/v0"



root = tk.Tk()
root.title('APIC-EM Network-Devices-Config            --              created by:     tsterber@cisco.com')
...
# search for a specific device based on the MAC, IP, Hostname,.. and print info and config
def search_device():
    ...
    while n < int(devicecount):
        ...
        if feld[field] == wert:
            feld = (response_json["response"][n])
            mytext.insert(tk.END, "Device          Id      : " + feld["id"]+"\n")
            deviceid = feld["id"]
            mytext.insert(tk.END, "\n")
            mytext.pack(side=tk.LEFT)
        n += 1

    # get and print the device config
    deviceid = str(deviceid)
    url = apic_url + "/network-device/" + deviceid + "/config"
    data = requests.get(url,verify=False)
    data_json = data.json()     # create json dic
    #
    datei = "./config.tmp"
    #
    f = open(datei, "w")
    print (data_json["response"], file = f)    # write config in file
    f.close()
    #
    mytext.insert(tk.END, "\n")
    mytext.insert(tk.END, "- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -\n")
    mytext.insert(tk.END, "     Device Config:\n")
    mytext.insert(tk.END, "- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -\n")
    mytext.insert(tk.END, "\n")
    #
    f = open(datei, "r")
...
searchfield.set("Mgmt IP")         # default value
...
en.pack()
searchinput.set("40.0.2.1")

abstand = tk.Label(f1, bg = "orange", text="")
abstand.pack()

b3 = tk.Button(f1, text='show config of device    ', padx=20, command=search_device)
b3.pack()
...
f2.pack()
mytext.insert(tk.END, "program started           ")
mytext.pack(side=tk.LEFT)
